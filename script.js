(function(){
  ...
  // lazy-activate embedded media (images/videos) as they approach the viewport,
  // so the browser doesn't try to decode everything on load
  (function(){
    var lazyEls = document.querySelectorAll('.lazy-media');
    ...
    if('IntersectionObserver' in window){
      var ioLazy = new IntersectionObserver(function(entries){
        entries.forEach(function(e){ if(e.isIntersecting){ activate(e.target); ioLazy.unobserve(e.target); } });
      }, { rootMargin: '700px 0px', threshold: 0.01 });
      lazyEls.forEach(function(el){ ioLazy.observe(el); });
    } else {
      lazyEls.forEach(activate);
    }
  })();
